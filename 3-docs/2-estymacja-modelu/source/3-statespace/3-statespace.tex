...
Kluczowym pytaniem jest: dlaczego nie stosuje się standardowych metod matematycznych (takich jak postać kanoniczna sterowalna)? Odpowiedź jest prosta — metody kanoniczne generują \textbf{stany matematyczne}, które mogą nie mieć fizycznego odpowiednika. W tym przypadku pożądane jest, aby trzecia zmienna stanu ($x_3$) reprezentowała kąt belki ($\theta$), a nie przyspieszenie piłki. 

\subsection{Dekompozycja transmitancji}

Określona jest transmitancja całkowita $G(s)$, która wiąże wyjście (pozycję piłki $x$) z
wejściem (napięciem sterującym $U(s)$). Zgodnie z opisem z sekcji
poprzedniej:
\begin{align}
...
Przechodząc na dziedzinę czasu:

\begin{equation}
T_{\mathrm{serwo}} \dot{\theta}(t) + \theta(t) = u(t)
\end{equation}

Wyznaczając najwyższą pochodną ($\dot{\theta}$):
...
Z rozdziału drugiego wiadomo, że dynamika piłki w zależności od kąta belki opisana jest równaniem:
...
Wiadomo, że kąt belki związany jest z kątem serwa przez przekładnię mechaniczną:
...
Pożądane jest, aby wektor stanu przyjął poniższą postać:
...
Jest to przyspieszenie. Pobiera się je z równania \eqref{eq:ball_dynamics_with_mech}, które uwzględnia przekładnię mechaniczną:

\begin{equation}
\ddot{x} = k_{\mathrm{mech}} \cdot c_{\mathrm{ball}} \cdot \theta_{\mathrm{serwo}}
\end{equation}

Skoro $\theta_{\mathrm{serwo}} = x_3$ (zmienna stanu), to po podstawieniu:
...
Jest to prędkość kątowa serwa. Pobiera się ją z równania \eqref{eq:servo_dynamics}:
...
\begin{equation}
\begin{bmatrix} \dot{x}_1 \\ \dot{x}_2 \\ \dot{x}_3 \end{bmatrix} = \begin{bmatrix} 0 & 1 & 0 \\ 0 & 0 & c_{\mathrm{ball}} \\ 0 & 0 & -\frac{1}{T_{\mathrm{serwo}}} \end{bmatrix} \begin{bmatrix} x_1 \\ x_2 \\ x_3 \end{bmatrix} + \begin{bmatrix} 0 \\ 0 \\ \frac{1}{T_{\mathrm{serwo}}} \end{bmatrix} u
\end{equation}

Oraz równanie wyjścia (mierzona jest pozycja $x$):

\begin{equation}
y = \begin{bmatrix} 1 & 0 & 0 \end{bmatrix} \begin{bmatrix} x_1 \\ x_2 \\ x_3 \end{bmatrix}
\end{equation}

Podstawiając wartości liczbowe stałych ( $c_{\mathrm{ball}} \approx 5{,}886$, $T_{\mathrm{serwo}} = 0{,}095$):

\begin{equation}
\begin{bmatrix} \dot{x}_1 \\ \dot{x}_2 \\ \dot{x}_3 \end{bmatrix} = \begin{bmatrix} 0 & 1 & 0 \\ 0 & 0 & 5{,}886 \\ 0 & 0 & -10,526 \end{bmatrix} \begin{bmatrix} x_1 \\ x_2 \\ x_3 \end{bmatrix} + \begin{bmatrix} 0 \\ 0 \\ 10,526 \end{bmatrix} u
\end{equation}
