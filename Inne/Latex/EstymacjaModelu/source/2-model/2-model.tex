...
Wał serwa jest połączony z belką za pośrednictwem mechanicznego połączenia. 
Zakładając idealne połączenie, obowiązuje 
...
gdzie:
\begin{itemize}
  \item $r_{\mathrm{serwo}}$ -- promień dźwigni na wale serwa,
  \item $\theta_{\mathrm{serwo}}$ -- kąt obrotu serwa $[\mathrm{rad}]$,
  \item $r_{\mathrm{belka}}$ -- promień dźwigni na wale belki($15\,\mathrm{cm}$),
  \item $\theta_{\mathrm{belka}}$ -- kąt wychylenia belki $[\mathrm{rad}]$.
\end{itemize}
...
Transmitancja całego systemu wynosi iloczyn transmitancji na każdym odcinku:

\[
G_{\mathrm{total}}(s) = \frac{X(s)}{U(s)} = G_{\mathrm{serwo}}(s) \cdot k_{mech} \cdot G_{\mathrm{ball}}(s)
\]

\paragraph{Wyjaśnienie pojawienia się $k_{mech}$ w liczniku:}
...
Jednak $\Theta_{\mathrm{belka}}$ nie jest bezpośrednie wyjściem serwa. Musimy podstawić 
...
Transmitancja całego systemu łączy dynamikę serwa (czlon inercyjny I rzędu) 
z dynamiką piłeczki na belce (dwa całkowujące człony $s^2$), tworząc system 
trzeciego rzędu. Przełożenie mechaniczne $n$ amplifikuje efekt serwa na wychylenie 
belki, natomiast inercja serwa charakteryzowana przez $T_{\mathrm{serwo}}$ opóźnia 
odpowiedź całego systemu.
