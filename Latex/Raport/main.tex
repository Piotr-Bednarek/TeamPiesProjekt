...
\title{\textbf{System sterowania Ball on Beam}\\[0.5cm]
\large Raport z projektu zaliczeniowego}
...
System oblicza sredni blad (rolling average) z ostatnich 35 probek, co pozwala na monitorowanie uchybu ustalonego. Zakres regulacji wynosi 250mm, wiec 5\% = 12.5mm, a 1\% = 2.5mm. Wizualizacja na LEDach wykorzystuje te progi.

\begin{lstlisting}[caption={Sredni uchyb (Rolling Average) - main.c}, style=cstyle]
// Bufor sredniego uchybu
float err_buffer[AVG_ERR_SAMPLES];
uint8_t err_idx = 0;
float err_sum = 0.0f;

// --- Obliczanie Sredniego Bledu (Rolling Average) ---
float current_abs_err = (current_error < 0) ? -current_error : current_error;

err_sum -= err_buffer[err_idx];       // Odejmij najstarsza probke
err_buffer[err_idx] = current_abs_err; // Zapisz nowa
err_sum += current_abs_err;           // Dodaj nowa
...
\begin{lstlisting}[caption={Wizualizacja uchybu na LEDach (leds.c)}, style=cstyle]
void UpdateErrorLEDs_5LED(float error) {
    // Wylacz wszystkie LEDy (PE2..PE6)
