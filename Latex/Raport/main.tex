...
\hline
Jan Andrzejewski \par Mateusz Banaszak \par Piotr Bednarek & \textbf{Raport z projektu zaliczeniowego} \par Ball on Beam & Data wyk.:\par 18.01.2026\\
\hline
Grupa środa 13:30  & &  \\
\hline
\end{tabular}
...
\subsection{Zakres i cel projektu}

Za cel projektu obrano zbudowanie stanowiska służacego do sterowania obiektem dynamiczym "Ball on beam". Za cel przy implementacji zostało przyjęte spełnienie dodatkowych wymogów projektowych opisanych w następnym rozdziale. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{img/stanowisko.png}
    \caption{Stanowisko}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{img/desktop.png}
...
\begin{itemize}
    ...
    \item Diody LED - 1x zielona, 2x czerwona, 2x żółta 
    \item 5x tranzystory npn pn2222
    \item 5x rezystory 2k$\Omega$
    \item Potencjometr 
    \item Zasilacz 5 V
\end{itemize}
...
Poniżej przedstawiono wymagania projektowe, które spełnia projekt, wraz z fragmentami kodu dokumentującymi ich implementację.

%------------------------------------------------------------------------------
\subsection{Pomiar zmiennej regulowanej ze stałym okresem próbkowania}
%------------------------------------------------------------------------------

System dokonuje pomiaru regulowanej zmiennej (pozycji kulki) ze stałym okresem próbkowania zdefiniowanym jako \texttt{PID\_DT\_MS = 30ms}. Zapewnia to funkcja \texttt{osDelay()} systemu FreeRTOS wywołana na końcu pętli sterowania.

\begin{lstlisting}[caption={Stały okres próbkowania (main.h)}, style=cstyle]
...
\begin{lstlisting}[caption={Implementacja stałego okresu w pętli sterowania (main.c)}, style=cstyle]
/* Infinite loop */
for (;;) {
    loop_counter++;
...
System zapewnia bezpieczne sterowanie poprzez zdefiniowanie twardych limitów kąta serwa w pliku \texttt{main.h} oraz ich egzekwowanie w kodzie sterowania.

\begin{lstlisting}[caption={Definicje limitów bezpieczeństwa serwa (main.h)}, style=cstyle]
// --- Servo Configuration ---
...
\begin{lstlisting}[caption={Saturacja wyjścia PID (main.c)}, style=cstyle]
...
System oblicza średni błąd (rolling average) z ostatnich 35 próbek, co pozwala na monitorowanie uchybu ustalonego. Zakres regulacji wynosi 250 mm, więc 5\% = 12,5 mm, a 1\% = 2,5 mm.

Wartość średniego uchybu jest przesyłana przez UART i wyświetlana w czasie rzeczywistym w aplikacji desktopowej na panelu metryk oraz na wykresie. Umożliwia to bieżącą ocenę jakości regulacji bez konieczności analizy surowych danych.

\begin{lstlisting}[caption={Obliczenie uchybu z 35 (main.c)}, style=cstyle]
// --- Obliczanie Średniego Błędu (Rolling Average) ---
		...
		err_sum += current_abs_err;           // Dodaj nową próbkę
...
%------------------------------------------------------------------------------
\subsection{Zadawanie wartości referencyjnej za pomocą komunikacji szeregowej lub urządzenia wejścia}
%------------------------------------------------------------------------------

Wartość referencyjna może być zadawana na dwa sposoby:
\begin{itemize}
    \item Przez komunikację szeregową UART (komenda \texttt{S:wartosc})
    \item Przez potencjometr podłączony do wejścia ADC (PA0)
\end{itemize}

\begin{lstlisting}[caption={Obsługa komendy setpointu przez UART (main.c)}, style=cstyle]
...
%------------------------------------------------------------------------------
\subsection{Podgląd sygnałów przez komunikację szeregową}
%------------------------------------------------------------------------------

System wysyła dane pomiarowe przez UART w formacie tekstowym z sumą kontrolną CRC-8. Dane zawierają: dystans surowy (D), setpoint (Z), kąt serwa (A), dystans filtrowany (F), błąd (E), średni błąd (V) oraz status sensora (S).

\begin{lstlisting}[caption={Wysyłanie danych przez UART z CRC (main.c)}, style=cstyle]
char data_buffer[64];
...
%------------------------------------------------------------------------------
\subsection{Podział kodu na moduły i dokumentacja Doxygen}
%------------------------------------------------------------------------------

Kod źródłowy jest podzielony na osobne moduły o jasno zdefiniowanych funkcjonalnościach. Każdy moduł posiada dokumentację zgodną ze standardem Doxygen.

\textbf{Struktura modułów STM32:}
\begin{itemize}
    \item \texttt{main.c/h} -- główna aplikacja, konfiguracja peryferiów
    \item \texttt{pid.c/h} -- wrapper na CMSIS DSP PID
    \item \texttt{servo\_pid.c/h} -- własna implementacja PID z Anti-Windup
    \item \texttt{filters.c/h} -- filtry cyfrowe (Mediana, EMA, 1-Euro)
    \item \texttt{crc8.c/h} -- obliczanie sumy kontrolnej CRC-8
    \item \texttt{leds.c/h} -- wizualizacja błędu na LEDach
    \item \texttt{calibration.c/h} -- kalibracja sensora
\end{itemize}

\begin{lstlisting}[caption={Przykład dokumentacji Doxygen (pid.c)}, style=cstyle]
/**
 ******************************************************************************
 * @file    pid.c
...
Aplikacja desktopowa została napisana w języku Python z wykorzystaniem frameworka PySide6 (Qt). Umożliwia ona:
\begin{itemize}
    \item Połączenie z mikrokontrolerem przez port szeregowy
    \item Wizualizację sygnałów w czasie rzeczywistym (wykresy PyQtGraph)
    \item Wybór trybu (UI, sinus, śledzenie trajektorii) i zadawanie sygnału referencyjnego 
    \item Zadawanie wartości parametrów PID
    \item Nagrywanie i eksport sygnałów do CSV
    \item Sterowanie trybem pracy regulatora (Start/Stop)
\end{itemize}

\begin{lstlisting}[caption={Główna klasa aplikacji GUI (app.py)}, style=pythonstyle]
...
%------------------------------------------------------------------------------
\subsection{Skrypty do logowania sygnałów}
%------------------------------------------------------------------------------

Aplikacja desktopowa posiada wbudowany mechanizm nagrywania danych pomiarowych do plików CSV z możliwością eksportu.
...
%------------------------------------------------------------------------------
\subsection{Dodatkowe urządzenie wyjścia -- wyświetlacz LED}
%------------------------------------------------------------------------------

System posiada 5 diod LED (PE2--PE6) wizualizujących aktualny uchyb regulacji:
\begin{itemize}
    \item LED zielona (PE4) -- błąd w zakresie $\pm$1\% (2,5 mm)
    \item LEDy żółte (PE3, PE5) -- błąd w zakresie $\pm$5\% (12,5 mm)
    \item LEDy czerwone (PE2, PE6) -- błąd większy niż $\pm$5\%
\end{itemize}

\begin{lstlisting}[caption={Definicje pinow LED (main.h)}, style=cstyle]
// --- LED Error Bar ---
#define LED_ERR_PORT      GPIOE
#define LED_ERR_1_Pin     GPIO_PIN_2 //PE2
#define LED_ERR_2_Pin     GPIO_PIN_3 //PE3
#define LED_ERR_3_Pin     GPIO_PIN_4 //PE4
#define LED_ERR_4_Pin     GPIO_PIN_5 //PE5
#define LED_ERR_5_Pin     GPIO_PIN_6 //PE6
\end{lstlisting}

\begin{lstlisting}[caption={Update stanu LED (leds.c)}, style=cstyle]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{img/schematled.png}
    \caption{Schemat połączenia LED}
\end{figure}

%------------------------------------------------------------------------------
\subsection{Dodatkowe urządzenie wejścia -- potencjometr}
%------------------------------------------------------------------------------

System wykorzystuje potencjometr podłączony do wejścia analogowego PA0 (ADC1\_CH0) do zadawania wartości referencyjnej w trybie analogowym.
...
%------------------------------------------------------------------------------
\subsection{Protokół komunikacji z sumą kontrolną CRC-8}
%------------------------------------------------------------------------------

Komunikacja szeregowa wykorzystuje sumę kontrolną CRC-8 z wielomianem 0x07 dla zapewnienia integralności danych. Implementacja jest identyczna po stronie STM32 (C) i aplikacji desktopowej (Python).
...
Projekt spełnia wszystkie wymagania funkcjonalne i dodatkowe:

\begin{enumerate}
    \item \textbf{Pomiar ze stałym okresem} -- \texttt{osDelay(PID\_DT\_MS)} zapewnia 30 ms okres próbkowania
    \item \textbf{Bezpieczny zakres sterowania} -- limity serwa 60--140 stopni
    \item \textbf{Uchyb ustalony} -- rolling average z 35 próbek, uchyb ustalony na poziomie <1\%
    \item \textbf{Zadawanie wartości referencyjnej} -- UART (komenda S:) i potencjometr (ADC PA0)
    \item \textbf{Podgląd sygnałów} -- ramki UART z danymi D/Z/A/F/E/V/S + CRC-8
    \item \textbf{Modularność i Doxygen} -- 7 modułów C, pełna dokumentacja API
    \item \textbf{Kontrola wersji} -- repozytorium Git
    \item \textbf{Aplikacja desktopowa} -- PySide6 (Qt) z wykresami PyQtGraph
    \item \textbf{Logowanie sygnałów} -- eksport do CSV z timestampami
    \item \textbf{Urządzenie wyjścia} -- 5 diod LED (PE2--PE6)
    \item \textbf{Urządzenie wejścia} -- potencjometr na ADC1\_CH0
